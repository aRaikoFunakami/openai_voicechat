...
def synthesis():
    ...
    # リクエストをリレーする
    response = requests.request(method, url, headers=headers, json=data, params=request.args)
    ...
    relay_response = Response(response.content, status=response.status_code, headers=dict(response.headers))
